...
void CreateBackupFile(const map<string, int>& frequencyMap, const string& backupFilename) {
	...
	if (!inFile.is_open()) {
		cout << "Error: Could not open file CS210_Project_Three_Input_File.txt" << endl;
		cout << "Ensure the file exists in the correct location with proper permissions." << endl;
		return;
	}
